...
```ad-statblock
title: Adult Oblex
![](z_compendium/bestiary/ooze/token/adult-oblex.png#token)
...
**3/day each**: [charm person](z_compendium/spells/charm-person.md) (as 5th-level spell), [detect thoughts](z_compendium/spells/detect-thoughts.md), [hypnotic pattern](z_compendium/spells/hypnotic-pattern.md)
...
While memory drained, the target must roll a `d4` and subtract the number rolled from its ability checks and attack rolls. Each time the target is memory drained beyond the first, the die size increases by one: the `d4` becomes a `d6`, the `d6` becomes a `d8`, and so on until the die becomes a `d20`, at which point the target becomes [unconscious](z_compendium/rules/conditions.md#unconscious) for 1 hour. The effect then ends.
