...
```ad-statblock
title: Necrichor
![](z_compendium/bestiary/undead/token/necrichor.png#token)
...
***Rejuvenation.*** Unless its lifeless remains are splashed with holy water or placed in a vessel under the effects of the [hallow](z_compendium/spells/hallow.md) spell, the destroyed necrichor re-forms in `1d10` days, regaining all its hits points and appearing in the place it died or in the nearest unoccupied space.
...
***Pseudopod.*** *Melee Weapon Attack:* +5 to hit, reach 10 ft., one target. *Hit:* 5 (`1d6 + 2`) necrotic damage, and the target must succeed on a DC 14 Constitution saving throw or be [paralyzed](z_compendium/rules/conditions.md#paralyzed) until the start of the necrichor's next turn.
